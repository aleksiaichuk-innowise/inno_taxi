...
func (r PgRepository) UpdateOrderStatus(ctx context.Context, id string, status service_dto.Status) (service_dto.Order, error) {
	const query = `
		UPDATE orders
	...
	if err != nil {
		return service_dto.Order{}, fmt.Errorf("update order status query: %w", err)
	}
	...
	if err != nil {
		...
		return service_dto.Order{}, fmt.Errorf("update order status scan: %w", err)
	}
